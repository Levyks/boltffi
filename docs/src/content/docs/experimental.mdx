...
**Not a gap vs Swift:** async methods on foreign callback proxies are not exposed; Swift does not offer that surface either.

Configure under `[targets.dart]` — see [Configuration](/docs/configuration#dart-configuration) for `output`, `native_architectures`, and `type_mappings`.
