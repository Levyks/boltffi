...
Generates unified multiplatform Dart packages containing both Native Assets (`dart:ffi`) and WebAssembly (`dart:js_interop`) bindings using conditional exports.
Enable with `experimental = ["dart"]` or `boltffi generate dart --experimental` / `boltffi pack dart --experimental`.

**Supported export surface (current):**

| Area | Status |
|------|--------|
| Multiplatform Unified Package (`dart:ffi` + `dart:js_interop` conditional exports) | Yes |
| WebAssembly target (`wasm32-unknown-unknown` + Web Crypto support) | Yes |
| Self-contained ES module WASM loader (`.mjs`, zero TypeScript build step) | Yes |
| Records / enums | Yes |
| Free functions (sync + async) | Yes |
| Opaque classes (instance + static methods, `dispose()` + finalizers) | Yes |
| Dart-implemented callbacks (sync & async; structured error decoding) | Yes |
