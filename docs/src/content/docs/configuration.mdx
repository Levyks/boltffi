...
`boltffi generate dart --experimental` writes `{output}/{package.name}/` containing:

- `lib/{package}.dart` — generated bindings and runtime helpers
...
`boltffi pack dart --experimental` builds the Rust `cdylib` and copies it into
`{output}/{package.name}/native/<rust-triple>/`. Use `--regenerate` to refresh bindings in the same pack step.
...
Map custom types to host Dart types the same way as Swift and Kotlin:
...
On Dart these usually stay as `String` because the standard library does not ship UUID/URL types comparable to Swift's `UUID` / `URL`.

See [Experimental Features](/docs/experimental#dart) for the full supported export surface (including returned closures and foreign callback proxies).
