...
The `[targets.dart]` section is **experimental** (`experimental = ["dart"]` or CLI `--experimental`). It controls Dart package generation and Native Assets packaging.

### Output Directory

Dart packages are written under `dist/dart` by default:
...
`boltffi generate dart --experimental` writes `{output}/{dart_package}/` containing:

- `lib/{dart_package}.dart` — generated bindings and runtime helpers
- `pubspec.yaml` — package metadata and Native Assets deps
- `hook/build.dart` — Dart build hook that resolves the native library
- `dart_target.json` — artifact name for the hook

`{dart_package}` is the normalized pub package name (see below). `boltffi pack dart --experimental` builds the Rust `cdylib` and copies it into `{output}/{dart_package}/native/<rust-triple>/`. Use `--regenerate` to refresh bindings in the same pack step.

### Web (Browser) Support

When `[targets.wasm]` is also enabled (see [WASM Configuration](#wasm-configuration)), the generated Dart package additionally exposes a browser implementation under `lib/src/web/`, selected automatically over the native one at compile time when the app targets `dart.library.js_interop` — no code changes needed on the calling side to support both.

The browser implementation depends on a compiled JS module and a `.wasm` binary that have to be reachable at runtime by the app serving them, and there is no mechanism — Flutter's or plain Dart's — that lets a package deliver files into a consuming app's own `web/` directory automatically. So, once per app, you need to:

1. Copy the contents of `lib/src/web/` into your app's `web/` directory.
2. Add a small `<script type="module">` block to your app's `web/index.html` that starts loading the module.
3. Call and await `ensureInitialized()` once before using any other export from the package.

`WEB_SETUP.md`, written at the root of the generated package (deliberately not inside `lib/src/web/` itself — otherwise a literal "copy this folder" would also copy the doc into your app's public web assets), has the exact file names, `index.html` snippet, and import path for your specific package — copy it from there rather than from generic docs, since it already has your package's real names substituted in.

`ensureInitialized()` only waits for the module the `index.html` script tag already started loading; it doesn't trigger loading itself. This keeps loading eager and script-tag-driven rather than deferred until first use, so the browser can fetch the wasm module in parallel with the rest of the app's startup instead of serialized behind it.

### Package Name

Pub package names must be lowercase `[a-z0-9_]`. By default BoltFFI derives the name from `[package].name` and normalizes it the same way as TypeScript modules: non-alphanumeric characters become `_`, and a leading digit is prefixed with `_`.

```toml
[package]
name = "my-lib"   # generates pub package `my_lib`
```

Override the pub name when it should differ from the logical package identity:
...
Overrides are normalized too, so `package_name = "my-sdk"` still becomes `my_sdk`.

### Native Architectures

By default BoltFFI packages for the current desktop host architecture. Specify `native_architectures` to build a custom target matrix:

```toml
[targets.dart]
enabled = true
native_architectures = ["windows:x86_64"]
```

Values use Dart OS/arch names (for example `macos:arm64`, `linux:x64`, `windows:x86_64`). They map to Cargo triples such as `x86_64-pc-windows-msvc`.

### Type Mappings

Map custom types to host Dart types:

```toml
[targets.dart.type_mappings]
Uuid = { type = "String", conversion = "uuid_string" }
Email = { type = "String", conversion = "url_string" }
```

Available conversions:

- `uuid_string` — string form of a UUID-like custom type
- `url_string` — string form of a URL-like custom type

Dart mappings typically use `String`, since the standard library has no dedicated UUID/URL types.

See [Experimental Features](/docs/experimental#dart) for the full supported export surface.
